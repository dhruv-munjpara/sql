...
CHANGE salseman_id salesman_id INT;

insert into salesman(salseman_id,name,city,commission) values
...
round(100-((purch_amt/6000)*100),2) as unachived_percentage from orders
...
select round(sum(purch_amt),2) as total_purchase_amount from orders;

/*5write a SQL query to find the highest purchase amount ordered by each customer. Return customer ID, maximum purchase amount.*/
 
 select customer_id,max(purch_amt) as max_purchse_amonut from orders group by customer_id;
...
select d.department_name,avg(e.salary) as average_salary, count(e.commission_pct)
as employees_with_commission from employees e join departments d on
...
FROM employees e WHERE e.job_id = (SELECT job_id FROM employees
WHERE employee_id = 169) AND e.employee_id <> 169;
...
CALL AddNewEmployee(
    300,                -- employee_id (must be unique)
    'Dhruv',
    'Munjpara',
    'dhruv@test.com',
    '9999999999',
...
CALL PromoteEmployee(150, 8000.00, 'SA_MAN');
